...
import { TW, TL, TR, TB, FH, NF, ROOF_Y, PG, BPF, sr, ri, pk } from './constants.js';
import { FD, OD } from './floors.js';
import { HN, HC, AN, AC, BP2, BN2, HM, AM, BM, CWN, CWM } from './npcs.js';
...
export function genWorld(){
  ...
  for(let i=0;i<NF;i++){
    ...
    for(let n=0;n<nT;n++){const roll=sr();
      if(roll<0.5){const convo=HM[Math.floor(sr()*HM.length)];S.npcs.push({type:'h',x:TL+100+sr()*(TW-200),y:fy-48,w:24,h:48,vx:0,vy:0,spd:1+sr()*1.5,color:pk(HC),name:pk(HN),fr:sr()>0.5,st:'idle',bob:sr()*10,at:0,floor:i,onF:true,convo,ci:0})}
      else if(roll<0.95||alienCount>=3){const convo=BM[Math.floor(sr()*BM.length)];S.npcs.push({type:'b',x:TL+180+sr()*(TW-360),y:fy-48,w:24,h:48,vx:0,vy:0,spd:0.65,pal:pk(BP2),name:pk(BN2),fr:sr()>0.5,st:'idle',bob:sr()*10,at:0,floor:i,lp:sr()*6,jt:180+Math.floor(sr()*180),onF:true,convo,ci:0})}
      else{alienCount++;const convo=AM[Math.floor(sr()*AM.length)];S.npcs.push({type:'a',x:TL+150+sr()*(TW-300),y:fy-48,w:24,h:48,vx:0,vy:0,spd:1+sr()*1.2,color:pk(AC),name:pk(AN),fr:sr()>0.5,st:'idle',bob:sr()*10,at:0,floor:i,onF:true,convo,ci:0})}
    }
